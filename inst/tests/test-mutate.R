...
test.table <- introspectTable(session, "test")

query <- session$query(test.table$b)
expect_error(query$execute(mutable = TRUE)$getAll())

result <- session$query(test.table$a)$execute(mutable = TRUE)$getAll()
...
result.df <- session$query(test.table$a)$execute()$getAll()
