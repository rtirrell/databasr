...
# Test 'out-of-order' join, in which the first table used in a JOIN is 
# the first table used in SELECT.
statement <- prepare_sql('SELECT
  `%database`.`databasr_test_1`.`i1` AS `i1`, `%database`.`databasr_test_1`.`v1` AS `v1`
FROM
  `%database`.`databasr_test_1`
JOIN
  `%database`.`databasr_test_2`
ON
  CONCAT(`%database`.`databasr_test_1`.`v1`, `%database`.`databasr_test_2`.`v1`);'
)

q4 <- session$select(db1)$join(concat(db1$v1, db2$v1))
expect_equal(q4$sql(), statement)